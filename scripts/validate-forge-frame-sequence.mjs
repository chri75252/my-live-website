...
if (!integration.includes('hero-reveal-match-v2.css') || !integration.includes('hero-reveal-match-v4.css') || !integration.includes('hero-3d-reveal-match-v4.js')) fail('V4 recovery integration is not loaded by index.html.');
if (integration.includes('hero-3d-reveal-match-v2.js') || integration.includes('hero-3d-reveal-match-v3.js') || integration.includes('hero-reveal-match-v3.css')) fail('A legacy hero renderer remains active in index.html.');
if (!integration.includes('data-scrub-ready="false"')) fail('Reveal scrub-readiness state is missing from index.html.');

const combined = await Promise.all([
  read('js/forge-frame-sequence.js'), read('js/forge-intro.js'), read('js/hero-3d-reveal-match-v4.js')
...
for (const required of ['drawProgress(', 'scrubReady', 'nearestLoadedBeforeReady', 'tbm:hero-lifecycle', 'applyHeroLifecycle', 'createIrregularShellGeometry']) {
  if (!combined.includes(required)) fail(`Required V4 implementation term missing: ${required}`);
}

console.log('Frame-sequence validation passed: 64 MP4-derived frames, verified 159/160 cutoff, protected homepage files intact, one reveal scheduler, and V4 recovery integration present.');
