"""Build, render and export the unified TBM V8 live-WebGL scene in Blender 5.2.

Usage from repository root:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.2\\blender.exe" --background --python blender/reference-match-v7/scripts/build_reference_match_v7.py -- --render keyframes
  & "C:\\Program Files\\Blender Foundation\\Blender 5.2\\blender.exe" --background --python blender/reference-match-v7/scripts/build_reference_match_v7.py -- --render reveal
  & "C:\\Program Files\\Blender Foundation\\Blender 5.2\\blender.exe" --background --python blender/reference-match-v7/scripts/build_reference_match_v7.py -- --render cards

The same named, animated scene supplies approval stills, fallback posters and the
live GLB used continuously across the scroll reveal and homepage hold.
"""
...
CONTRACT_PATH = ROOT / "blender/reference-match-v8/config/scene-contract.json"
...
MATERIAL_PROFILES = {
    "core": {
        "metallic": .72, "roughness": .17, "noise_scale": 2.2,
        "noise_detail": 2.0, "bump_strength": .055, "bump_distance": .018,
        "rough_min": .14, "rough_max": .20, "coat": .24,
        "coat_roughness": .16, "anisotropy": 0.0,
    },
    "polished": {
        "metallic": .98, "roughness": .14, "noise_scale": 1.8,
        "noise_detail": 1.5, "bump_strength": .025, "bump_distance": .008,
        "rough_min": .10, "rough_max": .18, "coat": .08,
        "coat_roughness": .12, "anisotropy": .18,
    },
    "network": {
        "metallic": .96, "roughness": .19, "noise_scale": 2.4,
        "noise_detail": 1.5, "bump_strength": .018, "bump_distance": .006,
        "rough_min": .16, "rough_max": .23, "coat": .04,
        "coat_roughness": .18, "anisotropy": .08,
    },
    "forged": {
        "metallic": .95, "roughness": .33, "noise_scale": 4.0,
        "noise_detail": 3.0, "bump_strength": .16, "bump_distance": .07,
        "rough_min": .28, "rough_max": .38, "coat": 0.0,
        "coat_roughness": .28, "anisotropy": 0.0,
    },
}


def metal_material(name: str, base: str, profile_name: str):
    ...
    texture.inputs["Roughness"].default_value = .52
...
def add_segmented_orbit(name, major, rotation, material, target, parent, reveal_start):
    ...
    for segment_index in range(segment_count):
        ...
        for point_index in range(points_per_segment):
            ...
            points.append(Vector((major * math.cos(angle), major * math.sin(angle), 0)))
        origin = points[0]
        arc = add_curve(
            f"{name}_Segment_{segment_index:02d}",
            [point - origin for point in points],
            material,
            target,
            bevel=.055,
        )
        ...
        keyframe_transform(arc, segment_start + 5, scale=(1, 1, 1))
    return group
...
def add_progressive_cage_edges(source, material, target):
    """Build independently growing mesh struts that survive glTF export."""
    result = []
    ...
    for index, edge in enumerate(ordered_edges[::6]):
        start_point = vertices[edge.vertices[0]]
        end_point = vertices[edge.vertices[1]]
        direction = end_point - start_point
        midpoint = (start_point + end_point) * .5
        ...
        line.name = f"Network_Edge_{index:03d}"
        line.rotation_mode = "QUATERNION"
        line.rotation_quaternion = direction.to_track_quat("Z", "Y")
        line.rotation_mode = "XYZ"
        line.data.materials.append(material)
        move_to(line, target)
        start = 108 + (index % 42)
        end = start + 14
        keyframe_transform(line, 1, scale=(1, 1, .001))
        keyframe_transform(line, start, scale=(1, 1, .001))
        keyframe_transform(line, end - 2, scale=(1, 1, 1.04))
        keyframe_transform(line, end, scale=(1, 1, 1))
        result.append(line)
    return result
...
def configure_scene():
    ...
    scene.cycles.samples = CONTRACT["render"]["approvalSamples"]
    ...
    scene.view_settings.exposure = math.log2(CONTRACT["toneMappingExposure"])
...
def build_scene():
    ...
    vfx_electric = make_collection("VFX_ELECTRIC")
    ...
    ember = emissive_material("M_Spark_Energy", PALETTE["gold"], 7.0)
    energy = emissive_material("M_Electric_Energy", PALETTE["energy"], 10.0)
    ...
    keyframe_transform(core, 1, location=(0, 0, -.90), rotation=(.18, -.42, .08), scale=(.32, .32, .32))
    keyframe_transform(core, 18, location=(0, 0, -.68), rotation=(.20, -.38, .10), scale=(.42, .42, .42))
    keyframe_transform(core, 34, location=(0, 0, -.34), rotation=(.22, -.32, .13), scale=(.56, .56, .56))
    keyframe_transform(core, 52, location=(0, 0, -.08), rotation=(.24, -.27, .16), scale=(.70, .70, .70))
    ...
    for index, (major, rotation) in enumerate(orbit_specs):
        ...
        orbit = add_segmented_orbit(
            f"Inner_Orbit_{index + 1}", major, rotation, polished,
            geo_orbits, orbit_rig, reveal_start,
        )
    ...
    for index, point in enumerate(vertices[::10]):
        point *= 2.56
    ...
    look_at(floor_light, desktop_target)
...
def render_reveal(scene):
    ...
    # Sequence frames use Blender Eevee for predictable local generation time.
    # Approval stills remain Cycles renders; both paths use the same V7 scene,
    # materials, lights and 192-frame animation.
...
def render_v8_stills(scene, desktop_camera, mobile_camera):
    output = directory("artifacts/reference-match-v8/approval")
    scene.render.engine = "BLENDER_EEVEE"
    scene.render.image_settings.file_format = "PNG"
    scene.render.image_settings.color_mode = "RGBA"
    scene.render.resolution_x, scene.render.resolution_y = CONTRACT["render"]["approval"]
    for frame in CONTRACT["render"]["approvalFrames"]:
        scene.camera = desktop_camera
        scene.frame_set(frame)
        scene.render.filepath = str(output / f"desktop-{frame:03d}.png")
        bpy.ops.render.render(write_still=True)
    scene.render.resolution_x, scene.render.resolution_y = CONTRACT["render"]["approvalMobile"]
    for frame in CONTRACT["render"]["approvalFrames"]:
        scene.camera = mobile_camera
        scene.frame_set(frame)
        scene.render.filepath = str(output / f"mobile-{frame:03d}.png")
        bpy.ops.render.render(write_still=True)


def render_v8_posters(scene, desktop_camera, mobile_camera):
    output = directory(CONTRACT["web"]["root"])
    scene.render.engine = "BLENDER_EEVEE"
    scene.render.image_settings.file_format = "WEBP"
    scene.render.image_settings.color_mode = "RGBA"
    scene.render.image_settings.quality = 92
    ...
    scene.camera = desktop_camera
    scene.render.resolution_x, scene.render.resolution_y = CONTRACT["render"]["posterDesktop"]
    scene.render.filepath = str(output / "poster-desktop.webp")
    bpy.ops.render.render(write_still=True)
    scene.camera = mobile_camera
    scene.render.resolution_x, scene.render.resolution_y = CONTRACT["render"]["posterMobile"]
    scene.render.filepath = str(output / "poster-mobile.webp")
    bpy.ops.render.render(write_still=True)
...
def export_web_glb(*, optimize=False):
    ...
    shutil.copy2(CONTRACT_PATH, ROOT / CONTRACT["web"]["root"] / "scene-contract.json")
    return output


def main():
    scene, camera, core = build_scene()
    mobile_camera = bpy.data.objects["Camera_Mobile"]
    blend = ROOT / "blender/reference-match-v8/TBM_LIVE_V8.blend"
    blend.parent.mkdir(parents=True, exist_ok=True)
    bpy.ops.wm.save_as_mainfile(filepath=str(blend))
    if MODE == "build":
        return
    if MODE == "approval-stills":
        render_v8_stills(scene, camera, mobile_camera)
    elif MODE == "posters":
        render_v8_posters(scene, camera, mobile_camera)
    elif MODE == "glb":
        export_web_glb(optimize=False)
    elif MODE == "glb-optimized":
        export_web_glb(optimize=True)
    elif MODE == "all":
        render_v8_stills(scene, camera, mobile_camera)
        render_v8_posters(scene, camera, mobile_camera)
        export_web_glb(optimize=False)
    else:
        raise ValueError(f"Unknown V8 mode: {MODE}")
