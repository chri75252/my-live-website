...
def build_scene():
    ...
    # V10 electricity is a very short contact flash, never a long, exposed
    # lightning drawing. Long curves were rejected in the first delivery pass:
    # despite the small bevel, their high contrast still read as a white
    # zig-zag in the wide shot. Keep the visible tip within the contact zone,
    # use one brief secondary fork only on selected impacts, and warm the
    # emission so the effect reads as a forge discharge rather than an overlay.
    def create_contact_arc(name, start, end, start_frame, seed):
        rng = random.Random(seed)
        origin = Vector(start)
        contact = Vector(end)
        direction = contact - origin
        # Terminate the rendered accent before it spans the open void between
        # the band and core. The contact point gets the ember burst instead.
        visible_end = origin.lerp(contact, .32)
        points = []
        for segment in range(11):
            ...
            points.append(origin.lerp(visible_end, ratio) + jitter)
        arc = add_curve(name, points, energy, vfx_electric, bevel=.0022)
        ...
        keyframe_transform(arc, start_frame + 3, scale=(.001, .001, .001))
    ...
    for event_index, (name, start, end, frame) in enumerate(contact_events):
        ...
        # A second fork is reserved for the two stronger, close-up contacts;
        # it remains tiny and is deliberately not visible in the early wide shot.
        if event_index in (1, 2):
            create_contact_arc(
                f"{name}_Fork", start, tuple(Vector(end) + Vector((.04, 0, .025))),
