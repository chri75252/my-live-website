...
5. Run `docs/tbm-3d-handoff/scripts/verify_exact_pre_pr5.ps1`.
...
If any hash differs, restore the exact baseline using `docs/tbm-3d-handoff/scripts/restore_exact_pre_pr5.ps1` and stop to report the discrepancy before reveal work.
...
1. Run `docs/tbm-3d-handoff/scripts/create_reveal_worktree.ps1` or create an equivalent isolated worktree.
2. Create a backup reference/tag if needed.
3. Implement on `feature/forge-intro-reveal-v2`.
4. Run `docs/tbm-3d-handoff/scripts/verify_reveal_did_not_modify_baseline.ps1` after each meaningful commit.
