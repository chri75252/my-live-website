...
The following later Forge Gate files are removed because they did not exist at the restore point and can confuse future work:
...
Introduced the Premium V2 homepage, procedural Three.js armillary, pointer parallax, visibility pausing, reduced-motion support, CSS fallback, and shared Premium V2 UI.

### PR #2 — Secondary-page completion

**Result:** Successful and should remain.

Added consistent product, about, partnership, testimonial, blog/article, contact, FAQ, legal, and responsive layouts. It was scoped away from `body.home-v2`, so it did not need to alter the homepage hero.
...
Added geometry-aware camera fitting, smaller centred stage, faster movement, forge comets, trails, node pulse, bloom, and scroll-velocity acceleration.

What went wrong:

- The object was zoomed out too far.
- Post-processing and extra objects made the design look tacky.
...
- Too many changes were introduced simultaneously.

Critical lesson: do not combine framing, new geometry, new lights, bloom, material changes, node changes, and scroll-speed changes in one PR.

### PR #6 — Forge Gate reveal

**Failed structurally and visually.**

The final hero was rebuilt rather than handed off to the approved hero. The reveal was not reliably visible, and the new scene became the dominant implementation instead of an intro layer.

### PR #7 — Forge Gate correction

**Failed to recover the target visual.**

The shader-generated result did not match the old approved physical-material scene. Fixing technical causes did not restore the visual baseline.

### PR #8 — “Restore” direct hero

**Failed because it was another recreation, not a revert.**

It changed the node count, materials, lighting, and architecture. The result remained visibly different from the screenshot the user identified as correct.

Critical lesson: when a known-good Git commit exists, restore that commit’s files. Do not write a “similar” replacement.
...
Syntax validation and CI cannot determine whether the design is attractive or matches the user’s screenshot.

Future rule: no hero PR is merged until a screenshot or short recording of the branch is reviewed by the user.
...
Future rule: one visual hypothesis per branch.

### 5.4 The reveal and final hero were coupled

The correct architecture is a separate intro layer that hands off to an untouched final hero.

### 5.5 “Passed checks” was overstated as visual success

CI proves syntax and guardrails only. It does not prove visual fidelity, good motion, correct framing, acceptable materials, or browser rendering quality.

### 5.6 Scroll velocity was used as an animation multiplier

Wheel velocity can spike sharply and cause perceived strobing or skipped rotations.
...
Only the intro may use a sticky viewport:

```css
.forge-intro { height: 220svh; }
.forge-intro__sticky {
  position: sticky;
  top: 0;
  height: 100svh;
...
homepageHero.style.transform = `translateY(${(1 - handoff) * 24}px)`;
```
...
7. Do not add comet balls, comet trails, yellow Fresnel shells, broad bloom, external moving point-light balls, thick pedestal rings, scroll-velocity acceleration, or replacement globe shaders.
...
## 8. Recommended baseline validation
...
This prevents architecture drift but does not prove visual quality.
...
- `TBM_3D_HERO_HANDOFF_2026-07-12.md`
- `FRESH_CHAT_CONTINUATION_PROMPT.md`
- `baseline_manifest.json`
- `reference_1_desired_pre_pr5.png`
- `reference_2_current_incorrect.png`
- `reference_3_chat_context.png`
- `tbm_3d_handoff_reference_sheet.png`

---

## 11. Local Codex ChatGPT Bridge placement

This chat does not have access to the local Codex ChatGPT Bridge filesystem. Copy the downloadable handoff package into:
...
The text handoff files are also stored in the repository under `docs/tbm-3d-handoff/`.
