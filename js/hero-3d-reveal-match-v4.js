...
async function initialiseRevealMatchHero() {
  ...
  camera.position.set(0, 0.025, 10);
  ...
  let heroScrollStartY = null;
  let motionEnabled = true;
  let pageVisible = !document.hidden;
  let stageVisible = true;
  let rafId = 0;
  let motionPhase = 0;
  let heroLifecycle = root.dataset.tbmHeroLifecycle || 'suspended';
  let handoffProgress = 0;
  let lifecycleMotionAllowed = false;
  ...
  function setCardProgress(progress, scrollProgress) {
    cards.forEach((card, index) => {
      ...
      const local = lifecycleSettled ? 1 : 0;
  ...
  function applyScene(scrollProgress, phase) {
    ...
    const motionFactor = motionEnabled && lifecycleMotionAllowed && !reducedMotion.matches ? 1 : 0;
    ...
    sculpture.position.set(
      ...
      lerp(-0.07, -0.108, settle) + Math.sin(phase * 0.28) * 0.006 * motionFactor,
    ...
    camera.position.z = lerp(10, 9.86, settle);
  ...
  function applyHeroLifecycle(nextState, nextProgress = 0) {
    heroLifecycle = nextState || 'suspended';
    handoffProgress = Number.isFinite(nextProgress) ? clamp(nextProgress, 0, 1) : 0;
    root.dataset.tbmHeroLifecycle = heroLifecycle;

    const webglOpacity = heroLifecycle === 'active'
      ? 1
      : heroLifecycle === 'handoff-ready'
        ? smooth(0.84, 1, handoffProgress)
        : 0;
    root.style.setProperty('--tbm-hero-webgl-opacity', webglOpacity.toFixed(4));

    window.clearTimeout(settleTimer);
    lifecycleMotionAllowed = false;
    lifecycleSettled = false;
    heroScrollStartY = null;
    targetProgress = 0;
    currentProgress = 0;
    delete root.dataset.tbmHeroSettled;
    hero.classList.remove('is-pinned');
    resetPointer();

    if (heroLifecycle === 'active') {
      heroScrollStartY = window.scrollY;
      const settleDelay = reducedMotion.matches ? 0 : 420;
      settleTimer = window.setTimeout(() => {
        if (heroLifecycle !== 'active') return;
        lifecycleSettled = true;
        lifecycleMotionAllowed = true;
        root.dataset.tbmHeroSettled = 'true';
      }, settleDelay);
    }
  }

  function handleHeroLifecycle(event) {
    applyHeroLifecycle(event.detail?.state, event.detail?.progress);
  }

  window.addEventListener('tbm:hero-lifecycle', handleHeroLifecycle);
  applyHeroLifecycle(heroLifecycle, 0);
  ...
  function installNativeFallback() {
    const update = () => {
      ...
      if (heroScrollStartY === null) heroScrollStartY = window.scrollY;
      const travel = Math.max(window.innerHeight * 0.9, 640);
      targetProgress = clamp((window.scrollY - heroScrollStartY) / travel, 0, 1);
    };
    update();
  ...
  function frame(time) {
    ...
    if (motionEnabled && lifecycleMotionAllowed && !reducedMotion.matches) motionPhase += deltaSeconds;
  ...
  window.__tbmRevealMatchHero = heroApi;
  window.__tbmHeroV4 = heroApi;

  window.addEventListener('pagehide', () => {
    cancelAnimationFrame(rafId);
    window.clearTimeout(settleTimer);
    window.removeEventListener('tbm:hero-lifecycle', handleHeroLifecycle);
...
function createStudioEnvironment(renderer) {
  ...
  const environmentTarget = pmrem.fromScene(studio, 0.04, 0.1, 50);
